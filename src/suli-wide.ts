...
const amodel: AcousticModel = {
  ...
  constants: {
    ...
    SHARP_H: 3700,
    SHARP_L: 3300,
    // EXPERIMENTAL (suli-wide): in the original `suli` model, ACUTE (2500-2700)
    // straddles the High vowel (H=2600) and GRAVE (1200-1900) straddles the Low
    // vowel (L=1750) — so a consonant gliding toward those bands can land almost
    // on top of the adjacent vowel, with little to no pitch movement. Annie
    // Rialland's acoustic study of real Silbo Gomero (Phonology 22, 2005) found
    // the opposite design principle: consonant loci "frame" the vowel space
    // entirely from outside it, for maximum contrast. This widens ACUTE_H and
    // GRAVE_H so both loci clear the vowel range with a margin, while leaving
    // ACUTE_L unchanged (the README's own design intentionally allows Acute to
    // stay near the vowel when adjacent to /a/ or /o,u/ — only the High-vowel
...
export const SuliWideModel = new TextModel(amodel);